...
!devChains.includes(network.name)
    ...
    : describe("Raffle Unit Tests", async function () {
          ...
          describe("checkUpkeep", async function () {
              ...
              it("returns true if there are players, ETH paid, is open, and enough time has passed", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
          ...
          describe("performUpkeep", function () {
              it("only executes if checkUpkeep is true", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
          ...
          describe("fulfillRandomWords", function () {
              ...
              it("selects a winner, resets the lottery, and sends money", async function () {
                  ...
                  await new Promise(async (resolve, reject) => {
                      raffle.once("WinnerPicked", async () => {
                          console.log("WinnerPicked event occurred")
                          try {
